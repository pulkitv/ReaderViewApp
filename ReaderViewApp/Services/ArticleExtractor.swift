...
class ArticleExtractor {
    ...
    func extractArticle(from url: URL, completion: @escaping (Result<Article, Error>) -> Void) {
        ...
        webView.navigationDelegate = ArticleExtractionNavigationDelegate(url: url, completion: completion)
...
// Context to hold active web views during extraction
class ArticleExtractionContext {
    static let shared = ArticleExtractionContext()
    var activeExtractions: [URL: WKWebView] = [:]
    ...
    private func extractContent(from webView: WKWebView) {
        ...
        webView.evaluateJavaScript(script) { [weak self] result, error in
            ...
            if let error = error {
                ...
                ArticleExtractionContext.shared.activeExtractions.removeValue(forKey: self.url)
                return
            }
